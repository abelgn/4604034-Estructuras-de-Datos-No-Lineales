...
duration<double> ordenar_heap_sort(int* A, int TAM_ARREGLO) {
    high_resolution_clock::time_point inicio = high_resolution_clock::now();
    heap_sort(A, TAM_ARREGLO-1);
    high_resolution_clock::time_point fin = high_resolution_clock::now();
    duration<double> tiempo = duration_cast<duration<double> >(fin - inicio);
    return tiempo;
}
...
void experimentos(int tam, int reps, bool ms, bool hs, bool qs) {
    ...
    for (int i = 0; i < REPETICIONES; i++) {
        ...
        if (hs) {
            aux = copiar_arreglo(A, TAM_ARREGLO);
            cout << "Iniciando ordenamiento con HEAP SORT... " << flush;
            tiempo = ordenar_heap_sort(aux, TAM_ARREGLO);
            cout << "\tOrdenado. ";
            cout << "\tTiempo: " << tiempo.count() << "s" << endl;
            t_prom_hs = t_prom_hs + tiempo.count();
            delete [] aux;
        }
    ...
    t_prom_hs = t_prom_hs / REPETICIONES;
...
int main(int argc, char * argv[]) {
    if (argc != 4) {
        ...
        bool hs = incluirAlgoritmo(argv[3], 'h');
        experimentos(atol(argv[1]), atoi(argv[2]), ms, hs, qs);
    }
    return EXIT_SUCCESS;
}
